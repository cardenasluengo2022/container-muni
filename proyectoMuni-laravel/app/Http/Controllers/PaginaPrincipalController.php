...
class PaginaPrincipalController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        if (isset($paginaPrincipal)) {
            return Redirect::route('voyager.pagina-principal.edit', [$paginaPrincipal->id]);
            //return redirect()->action([ConfiguracionSitioController::class, 'edit']);
        }else{
            return Redirect::route('voyager.pagina-principal.create');
            //return redirect()->action([ConfiguracionSitioController::class, 'create']);
        }
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $arreglo = [];
        $arreglo2 = [];
        $noticias = DB::table('noticias')->whereNull('deleted_at')->orderBy('updated_at', 'asc')->get();
        $paginas = DB::table('paginas')->whereNull('deleted_at')->orderBy('updated_at', 'asc')->get();
        
        foreach ($noticias as $n) {
            $arreglo["noticias|".$n->slug] = "Noticia - ".$n->titular;
            $arreglo2["noticias|".$n->slug] = "Noticia - ".$n->titular;
        }

        foreach ($paginas as $p) {
            $arreglo["paginas|".$p->slug] = "Pagina - ".$p->titulo_menu;
            $arreglo2["paginas|".$p->slug] = "Pagina - ".$p->titulo_menu;
        }

        $portada_contenido = [];
        $segundaria_contenido = [];
        $ids = [];
        $ids2 = [];

        if (isset($dataTypeContent->portada_contenido)) {
            $limpio = str_replace(['"', '[', ']'], '', json_decode($dataTypeContent->portada_contenido, true));
        ...
        foreach ($ids as $valor) {
            if (isset($arreglo[$valor])) {
                $portada_contenido[$valor] = $arreglo[$valor];
            }
        }
        $arreglo = array_diff($arreglo, $portada_contenido);


        if (isset($dataTypeContent->segundaria_contenido)) {
            $limpio = str_replace(['"', '[', ']'], '', json_decode($dataTypeContent->segundaria_contenido, true));
            $limpio = explode(',', $limpio);
            $ids2 = $limpio;
        }

        foreach ($ids2 as $valor) {
            if (isset($arreglo2[$valor])) {
                $segundaria_contenido[$valor] = $arreglo2[$valor];
            }
        }

        $arreglo2 = array_diff($arreglo2, $segundaria_contenido);
